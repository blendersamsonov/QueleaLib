...
// DEPRECATED, и больше не поддерживается, поскольку не описывает направление скорости электрона в
// произвольном поле. Следует использовать *beta_or_eta* вместо данной функции.
fn beta(r: V, t: f64, f: &Box<Fn(V, f64) -> EM>) -> Option<V> {
    let em = f(r, t);
    let (ex, ey, ez) = (em.ex, em.ey, em.ez);
    let (bx, by, bz) = (em.bx, em.by, em.bz);
    ...
    let G = ex * bx + ey * by + ez * bz;
    //
    // здесь должно быть условие, при котором формулы для beta не дают "нормального" результата
    // (например, дают комплексный ответ). Однако из main.tex я не могу получить такого условия.
    // В то же время понятно, что при B \perp E и B > E мы не можем найти beta. Нет ли в формулах
    // (7)-(9) ошибки?
    if F < 0.0 || e2 == 0.0 {
        return None;
        //panic!("beta is ambiguous for given fields!");
    }
    //
    // проекции \mathbf v на ортогональный базис
    //
    // \mathbf v \cdot \mathbf E
    let vE = -f64::sqrt(0.5 * (F + f64::sqrt(F * F + 4.0 * G * G)));
    // \mathbf v \cdot [\mathbf E, \mathbf B]
    let vEB = b2 - G * G / (vE * vE);
    // \mathbf v \cdot [\mathbf E, [\mathbfE, \mathbf B]]
    let vEEB = -G * (vE - e2 / vE);
    //
    ...
    let EBvEB = (EB.0 * vEB,
                 EB.1 * vEB,
                 EB.2 * vEB);
    let EEBvEEB = (EEB.0 * vEEB,
                   EEB.1 * vEEB,
                   EEB.2 * vEEB);
    // компоненты \mathbf v получаем нормировкой
    let a = (EvE.0 / e2,
             EvE.1 / e2,
             EvE.2 / e2);
    let EB2 = EB.0 * EB.0 + EB.1 * EB.1 + EB.2 * EB.2;
    let b = if EB2 != 0.0 {
        (EBvEB.0 / EB2,
         EBvEB.1 / EB2,
         EBvEB.2 / EB2)
    ...
    let EEB2 = EEB.0 * EEB.0 + EEB.1 * EEB.1 + EEB.2 * EEB.2;
    let c = if EEB2 != 0.0 {
        (EEBvEEB.0 / EEB2,
         EEBvEEB.1 / EEB2,
         EEBvEEB.2 / EEB2)
    ...
    Some((a.0 + b.0 + c.0, a.1 + b.1 + c.1, a.2 + b.2 + c.2))
}
...
impl BetaEta {
    fn unwrap(&self) -> V {
        let y = match *self { BetaEta::Beta(x) => x,
                              BetaEta::Eta(x)  => x };
        y
    }
}

fn beta_or_eta(r: V, t: f64, f: &Box<Fn(V, f64) -> EM>) -> BetaEta {
    let em = f(r, t);
    let (ex, ey, ez) = (em.ex, em.ey, em.ez);
    let (bx, by, bz) = (em.bx, em.by, em.bz);
    let e2 = ex * ex + ey * ey + ez * ez;
    ...
    let mut EB =
        (ey * bz - ez * by,
         ez * bx - ex * bz,
         ex * by - ey * bx);
    let mut EB2 = EB.0 * EB.0 + EB.1 * EB.1 + EB.2 * EB.2;
    if F <= 0.0 && (G == 0.0 || G * G < 1e-6 * e2 * b2) {
        // eta case
        if b2 != 0.0 {
            return BetaEta::Eta((EB.0 / b2, EB.1 / b2, EB.2 / b2));
        } else {
            return BetaEta::Eta((0.0, 0.0, 0.0));
        }
    } else {
        // beta case
        //
        ...
        if a.0 * a.0 + a.1 * a.1 + a.2 * a.2 > 1.0 {
            return BetaEta::Beta((a.0, a.1, a.2));
        } else {
            let mut b = if EB2 != 0.0 {
                (EBvEB.0 / EB2,
                 EBvEB.1 / EB2,
                 EBvEB.2 / EB2)
            } else {
            ...
            if b.0 * b.0 + b.1 * b.1 + b.2 * b.2 > 1.1 {
                b = (0.0, 0.0, 0.0);
            }
            let EEB2 = EEB.0 * EEB.0 + EEB.1 * EEB.1 + EEB.2 * EEB.2;
            let mut c = if EEB2 != 0.0 {
                (EEBvEEB.0 / EEB2,
                 EEBvEEB.1 / EEB2,
                 EEBvEEB.2 / EEB2)
            } else {
            ...
            if c.0 * c.0 + c.1 * c.1 + c.2 * c.2 > 1.1 {
                c = (0.0, 0.0, 0.0);
            }
            return BetaEta::Beta((a.0 + b.0 + c.0, a.1 + b.1 + c.1, a.2 + b.2 + c.2));
        }
    }
}

//Analytical stationary solution
fn beta_solver(p0: Particle, f: Box<Fn(V, f64) -> EM>, dt: f64, n: usize) -> Vec<Particle> {
    let mut Th: Vec<Particle> = Vec::new();
    let mut p = p0;
    let mut ptmp = p.clone();   
    fn beta(e: [f64; 3], b: [f64; 3]) -> [f64; 3] {
        let mut ux: f64 = 0.0;
        let mut uy: f64 = 0.0;
        let mut uz: f64 = 0.0;
        let EXB = [e[1] * b[2] - e[2] * b[1], e[2] * b[0] - e[0] * b[2], e[0] * b[1] - e[1] * b[0]];
        let E2 = e[0] * e[0] + e[1] * e[1] + e[2] * e[2];
        let B2 = b[0] * b[0] + b[1] * b[1] + b[2] * b[2];
        let EB = e[0] * b[0] + e[1] * b[1] + e[2] * b[2];
        
        if f64::abs(E2) < 0.000001 && f64::abs(B2) > 0.000001 {
            let a = 1.0 / f64::sqrt(B2);
            ux = b[0] * a;
            uy = b[1] * a;
            uz = b[2] * a;
        }
        if f64::abs(B2) < 0.000001 && f64::abs(E2) > 0.000001 {
            let a = 1.0 / f64::sqrt(E2);
            ux = e[0] * a;
            uy = e[1] * a;
            uz = e[2] * a;
        }
        if f64::abs(E2) < 0.000001 && f64::abs(B2) < 0.000001 {
            ...
            if f64::abs(EB * EB - E2 * B2) > 0.000001 { //Not collinear
                let A = (E2 + B2 - f64::sqrt((E2 - B2) * (E2 - B2) - 4.0 * EB * EB)) / (2.0 * (E2 * B2 - EB * EB));
                let mut B = 0.0;
                if A - A * A * B2 > 0.0 {
                    B = f64::sqrt(A - A * A * B2);
                }
                let mut C = 0.0;
                if f64::abs(B) > 0.000001 {
                    C = A * A * EB / B;
                }
                else { //True in wave: EB = 0, E^2 = B^2 -> A = 1/B^2 -> B = 0
                    C = 0.0; //Not sure
                }
                ux = A * EXB[0] + B * e[0] + C * b[0];
                uy = A * EXB[1] + B * e[1] + C * b[1];
                uz = A * EXB[2] + B * e[2] + C * b[2];
            }
            else {
                let a = 1.0 / f64::sqrt(E2);
                ux = e[0] * a;
    ...
    for i in 0..n {
        //Runge-Kutta 2nd order (Euler like)
        // Мне кажется, что эта схема точнее схемы Эйлера? / Е. Н.
        let em = f((p.x, p.y, p.z), (i as f64) * dt);
        let u1 = beta([-em.ex, -em.ey, -em.ez], [-em.bx, -em.by, -em.bz]);
        ptmp.x += dt * u1[0];
        ptmp.y += dt * u1[1];
        ptmp.z += dt * u1[2];
        let em = f((ptmp.x, ptmp.y, ptmp.z), (1.0 + i as f64) * dt);
        let u2 = beta([-em.ex, -em.ey, -em.ez], [-em.bx, -em.by, -em.bz]);
        p.ux = 0.5 * (u1[0] + u2[0]);
        p.uy = 0.5 * (u1[1] + u2[1]);
        p.uz = 0.5 * (u1[2] + u2[2]);
        p.x += dt * p.ux;
...
// DEPRECATED, см. комментарий к функции beta(). Следует использовать *beta_eta_solver* вместо
// данной функции.
// beta-траектория, то есть решение уравнения
// d r / dt = beta.
// n - желаемое число шагов. Если в процессе движения координата попадает в область, где
// beta-теория неприменима, вычисления останавливаются в возвращается массив значений r до данного
// момента.
// Нет большого смысла принимать в качестве начального условия частицу и
// возвращать вектор частиц --- ux, uy, uz в них не имеют смысла.
fn beta_solver_new(r0: V, t0: f64, f: &Box<Fn(V, f64) -> EM>, dt: f64, n: usize) -> Vec<V> {
    let mut vec = Vec::new();
    vec.push(r0);
    for i in 0..(n - 1) {
        // Метод Рунге--Кутты четвёртого порядка, см. русскую вики.
        let r = match vec.last() { Some(x) => *x, // dereferencing because last() returns reference
                                 None    => unreachable!() };
        ...
        let k1 = b1.unwrap();
        let r1 = (r.0 + k1.0 * 0.5 * dt,
                  r.1 + k1.1 * 0.5 * dt,
                  r.2 + k1.2 * 0.5 * dt);
        let b2 = beta(r1, t + 0.5 * dt, &f);
        if b2.is_none() {
            return vec;
        }
        let k2 = b2.unwrap();
        let r2 = (r.0 + k2.0 * 0.5 * dt,
                  r.1 + k2.1 * 0.5 * dt,
                  r.2 + k2.2 * 0.5 * dt);
        let b3 = beta(r2, t + 0.5 * dt, &f);
        if b3.is_none() {
            return vec;
        }
        let k3 = b3.unwrap();
        let r3 = (r.0 + k3.0 * dt,
                  r.1 + k3.1 * dt,
                  r.2 + k3.2 * dt);
        let b4 = beta(r3, t + dt, &f);
        if b4.is_none() {
            return vec;
        }
        let k4 = b4.unwrap();
        vec.push((r.0 + dt * (k1.0 + k2.0 + k3.0 + k4.0) / 6.0,
                  r.1 + dt * (k1.1 + k2.1 + k3.1 + k4.1) / 6.0,
                  r.2 + dt * (k1.2 + k2.2 + k3.2 + k4.2) / 6.0));
    }
    vec
} 
...
// beta-eta-траектория, то есть решение уравнения
// d r / dt = beta (или eta, в зависимости от полей).
// n - число точек в возвращаемом решении.
// Точки несут информацио о поле (beta или eta) в данной (!), а не в промежуточной, точке.
fn beta_eta_solver(r0: V, t0: f64, f: &Box<Fn(V, f64) -> EM>, dt: f64, n: usize) -> Vec<BetaEta> {
    let mut vec = Vec::new();
    let mut r = r0;
    for i in 0..n {
        // Метод Рунге--Кутты четвёртого порядка, см. русскую вики.
        let t = t0 + i as f64 * dt;
        let b1 = beta_or_eta(r, t, &f);
        let wrapped_r = match b1 { BetaEta::Beta(_) => BetaEta::Beta(r),
                                   BetaEta::Eta(_)  => BetaEta::Eta(r) };
        vec.push(wrapped_r);
        let k1 = b1.unwrap();
        let r1 = (r.0 + k1.0 * 0.5 * dt,
                  r.1 + k1.1 * 0.5 * dt,
                  r.2 + k1.2 * 0.5 * dt);
        let k2 = beta_or_eta(r1, t + 0.5 * dt, &f).unwrap();
        let r2 = (r.0 + k2.0 * 0.5 * dt,
                  r.1 + k2.1 * 0.5 * dt,
                  r.2 + k2.2 * 0.5 * dt);
        let k3 = beta_or_eta(r2, t + 0.5 * dt, &f).unwrap();
        let r3 = (r.0 + k3.0 * dt,
                  r.1 + k3.1 * dt,
                  r.2 + k3.2 * dt);
        let k4 = beta_or_eta(r3, t + dt, &f).unwrap();
        r = ( r.0 + (k1.0 + 2.0 * k2.0 + 2.0 * k3.0 + k4.0) * dt / 6.0,
              r.1 + (k1.1 + 2.0 * k2.1 + 2.0 * k3.1 + k4.1) * dt / 6.0,
              r.2 + (k1.2 + 2.0 * k2.2 + 2.0 * k3.2 + k4.2) * dt / 6.0 );
    }
    vec
}
...
fn drive_particle(p0: Particle, f: Box<Fn(V, f64) -> EM>, t0: f64, dt: f64, n: usize, lambda: f64, rf_scheme: i32) -> Vec<Particle> {
    ...
    for i in 0..(n + 1) {
        ...
        if rf_scheme == 0 {
            ...
            /* this is BUG, не удаляю just for the record
             * if chi < 0.13333 {
                r = r * chi / 0.13333;
            }
            */
...
//Аналогично для beta_solver и drive_particle
#[no_mangle]
...
#[no_mangle]
pub extern "C"
fn beta_binding(r: *const c_double, t: c_double, ftype: c_int, nfparam: size_t, fparam: *const
    ...
    let v = match beta(r1, t as f64, &f) { Some(x) => x,
                                           None    => (0.0, 0.0, 0.0) };
    let vec = vec![v.0, v.1, v.2];
    let p = vec.as_ptr();
    std::mem::forget(vec);
    p as *const c_double
}
...
#[repr(C)]
pub struct c_BetaEta {
    is_beta: usize, // должен быть bool, но он почему-то неверно читается из Haskell (Haskell,
                    // по-видимому, использует int под Bool, а Rust - i1). Возможно, баг в llvm
                    // https://github.com/bos/llvm/issues/36)
    pvalue: *const c_double, // pointer to value
}

#[no_mangle]
pub extern "C"
fn c_beta_or_eta(r: *const c_double, t: c_double, ftype: c_int, nfparam: size_t, fparam: *const
                c_double) -> *const c_BetaEta {
    let slice_r = unsafe { std::slice::from_raw_parts(r as *const f64, 3usize) };
    let r1 = (slice_r[0], slice_r[1], slice_r[2]);
    let slice_fparam = unsafe { std::slice::from_raw_parts(fparam as *const f64, nfparam as usize) };
    let mut fp: Vec<f64> = Vec::new();
    ...
    let f = fields(ftype as i32, fp);
    let be = beta_or_eta(r1, t as f64, &f);
    let (is_beta, value) = match be { BetaEta::Beta(x) => (true,  vec![x.0, x.1, x.2]),
                                      BetaEta::Eta(x)  => (false, vec![x.0, x.1, x.2]) };
    let c_be = Box::new(c_BetaEta { is_beta: is_beta as usize, pvalue: value.as_ptr() as *const c_double });
    let p_c_be = c_be.as_ref() as *const c_BetaEta;
    std::mem::forget(value);
    std::mem::forget(c_be);
    p_c_be
}

// DEPRECATED, см. комментарий к функциям beta() и beta_solver_new().
#[no_mangle]
pub extern "C"
fn beta_solver_new_binding(r: *const c_double, t0: c_double, dt: c_double, n: size_t, ftype: c_int, nfparam:
                           size_t, fparam: *const c_double) -> *const Vlv {
    let slice_r = unsafe { std::slice::from_raw_parts(r as *const f64, 3usize) };
    let r0 = (slice_r[0], slice_r[1], slice_r[2]);
    let slice_fparam = unsafe { std::slice::from_raw_parts(fparam as *const f64, nfparam as usize) };
    let mut fp: Vec<f64> = Vec::new();
    for x in slice_fparam {
        fp.push(*x);
    ...
    let f = fields(ftype as i32, fp);
    let vec = beta_solver_new(r0, t0 as f64, &f, dt as f64, n as usize);
    let vlv = Box::new(Vlv { n: vec.len() as size_t, ptr: vec.as_ptr() as *const c_double });
    let pvlv = vlv.as_ref() as *const Vlv;
    std::mem::forget(vec);
    std::mem::forget(vlv);
    pvlv
}
...
#[no_mangle]
pub extern "C"
fn c_beta_eta_solver(r: *const c_double, t0: c_double, dt: c_double, n: size_t, ftype: c_int,
                     nfparam: size_t, fparam: *const c_double) -> *const c_BetaEta {
    let slice_r = unsafe { std::slice::from_raw_parts(r as *const f64, 3usize) };
    let r0 = (slice_r[0], slice_r[1], slice_r[2]);
    let slice_fparam = unsafe { std::slice::from_raw_parts(fparam as *const f64, nfparam as usize) };
    let mut fp: Vec<f64> = Vec::new();
    ...
    let f = fields(ftype as i32, fp);
    let track = beta_eta_solver(r0, t0 as f64, &f, dt as f64, n as usize);
    let mut cvec: Vec<c_BetaEta> = Vec::new();
    for b in track {
        let is_beta = match b { BetaEta::Beta(_) => true,
                                BetaEta::Eta(_)  => false };
        let r = b.unwrap();
        let cr = vec![r.0 as c_double, r.1 as c_double, r.2 as c_double];
        let pcr = cr.as_ptr();
        std::mem::forget(cr); // большая утечка памяти
        cvec.push( c_BetaEta { is_beta: is_beta as usize, pvalue: pcr as *const c_double } );
    }
    let p = cvec.as_ptr() as *const c_BetaEta;
    std::mem::forget(cvec);
    p
}
...
#[no_mangle]
pub extern "C" fn beta_solver_binding(s0: *const c_double, dt: c_double, n: size_t, ftype: c_int, nfparam:
                              size_t, fparam: *const c_double) -> *const c_double {
    let slice_s0 = unsafe { std::slice::from_raw_parts(s0 as *const f64, 6usize) };
    let s = Particle { x: slice_s0[0], y: slice_s0[1], z: slice_s0[2],
                       ux: slice_s0[3], uy: slice_s0[4], uz: slice_s0[5] };
    ...
    let vec = beta_solver(s, fields(ftype as i32, fp), dt as f64, n as usize);
    let mut plain_vec: Vec<f64> = Vec::new();
    for p in vec {
        plain_vec.push(p.x);
        plain_vec.push(p.y);
